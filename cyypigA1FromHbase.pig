...
finalRequests = LOAD 'hbase://RequestProducts' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage ('products:DumpDate products:UserId products:ECId products:ProductId products:ProductName products:ProductCategory products:ReqCount', '-gt=$StartDate -lt=$EndDate') AS (DumpTime:chararray, UserId:chararray, ECId:chararray, ProductId:chararray, ProductName:chararray, ProductCategory:chararray, ReqCount:long);

--- 計算並產生 Diff 欄位
validRequests = FOREACH finalRequests {
	DumpDateInDateTime = ToDate(DumpTime, 'yyyy-MM-dd');
	EndDateInDateTime = ToDate('$EndDate', 'yyyy-MM-dd');
	diff = DaysBetween(EndDateInDateTime, DumpDateInDateTime);
	GENERATE DumpDateInDateTime as DumpTime, UserId .., diff as TimeDiff;
}

validRequests = ORDER validRequests BY TimeDiff;
...
%declare faketime '2013-12-15T00:00:00.000-08:00'
