...
fittedObjectName_2 = FILTER regenData BY (ObjectName matches '.*面膜.*');
fittedObjectName_2 = FOREACH fittedObjectName_2 GENERATE * AS (UniqueId:chararray, DomainName:chararray, IPAddress:chararray, DumpTime:chararray, Referer:chararray, ECId:chararray, ProductId:chararray, Name:chararray, ObjectName:chararray);
...
STORE concatLabel INTO 'hbase://r1' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('cf:看過面膜');

concatLabel = foreach countingFilterResult_3 generate $0;
LOGS_GROUP= GROUP concatLabel ALL;
LOG_COUNT = FOREACH LOGS_GROUP GENERATE CONCAT('看過面膜', CONCAT('_',(chararray)'$stime')) as key, BagToTuple($1);
