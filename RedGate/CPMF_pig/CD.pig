...
dateFilterResult_4 = filter calculateDiffDay by Diffday <= (long)7 and Diffday >=0;
dateFilterResult_4 = FOREACH dateFilterResult_4 GENERATE * AS (UniqueId:chararray, DomainName:chararray, Url:chararray, IPAddress:chararray, DumpTime:chararray, Referer:chararray, SessionId:chararray, ECId:chararray, ProductId:chararray, Name:chararray, ObjectName:chararray, Date:datetime, DiffDay:chararray);

regenData = foreach dateFilterResult_4 generate UniqueId, DomainName, IPAddress, DumpTime, Referer, ECId, ProductId, Name, ObjectName;
fittedName_5 = FILTER regenData BY (ObjectName matches '.*面膜.*');
fittedName_5 = FOREACH fittedName_5 GENERATE * AS (UniqueId:chararray, DomainName:chararray, IPAddress:chararray, DumpTime:chararray, Referer:chararray, ECId:chararray, ProductId:chararray, Name:chararray, ObjectName:chararray);
...
STORE concatLabel INTO 'hbase://r1' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('cf:看過面膜且去過美妝社群網站');

concatLabel = foreach JoinResult_7 generate $0;
LOGS_GROUP= GROUP concatLabel ALL;
LOG_COUNT = FOREACH LOGS_GROUP GENERATE CONCAT('看過面膜且去過美妝社群網站', CONCAT('_',(chararray)'$stime')) as key, BagToTuple($1);
