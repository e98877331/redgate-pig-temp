...
fittedObjectName_2 = FILTER regenData BY (PGCatalogName matches '.*幼兒.*' OR PGCatalogName matches '.*孕婦.*' OR PGCatalogName matches '.*嬰.*' OR PGCatalogName matches '.*嬰幼.*');
fittedObjectName_2 = FOREACH fittedObjectName_2 GENERATE * AS (UniqueId:chararray, DomainName:chararray, IPAddress:chararray, DumpTime:chararray, Referer:chararray, ECId:chararray, ProductId:chararray, PGCatalogName:chararray, Name:chararray, ObjectName:chararray);

groupCountingData = group fittedObjectName_2 by UniqueId;
...
STORE concatLabel INTO 'hbase://r1' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('cf:家有小孩');

concatLabel = foreach countingFilterResult_3 generate $0;
LOGS_GROUP= GROUP concatLabel ALL;
LOG_COUNT = FOREACH LOGS_GROUP GENERATE CONCAT('家有小孩', CONCAT('_',(chararray)'$stime')) as key, BagToTuple($1);
